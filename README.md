...
Проект настроен для работы с GitHub Actions для автоматического запуска тестов при изменении кода.

### Рабочие процессы GitHub Actions

1. **Playwright Tests** - запускается при каждом push в основную ветку или открытии pull request:
   * Запускает тесты в Chrome по умолчанию
   * Генерирует и публикует отчеты Allure и Playwright
   * Сохраняет трассировку теста при неудаче

2. **Nightly Tests** - запускается каждую ночь в 2:00 UTC:
   * Запускает тесты во всех браузерах (Chrome, Firefox, Safari)
   * Тестирует все настроенные окружения
   * Генерирует и публикует полные отчеты

3. **Publish Allure Report** - запускается после завершения тестов:
   * Объединяет отчеты Allure из разных браузеров и окружений
   * Публикует сводный отчет на GitHub Pages

### Ручной запуск тестов

Вы можете вручную запустить тесты через GitHub Actions, используя опцию "workflow_dispatch" из интерфейса GitHub.

### Просмотр отчетов

Отчеты Allure публикуются на GitHub Pages и доступны по адресу:
`https://{ваш-github-username}.github.io/{имя-репозитория}/`

Также отчеты сохраняются как артефакты GitHub Actions и доступны через интерфейс GitHub.
